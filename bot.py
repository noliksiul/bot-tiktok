...
# Debes definir DATABASE_URL en Render con el formato:
# postgresql://USER:PASSWORD@HOST:PORT/DBNAME?sslmode=require
DATABASE_URL = os.getenv("postgresql://postgres:2011301546Acs@db.zndejlffvlznbhsfozst.supabase.co:5432/postgres")
if not DATABASE_URL:
...
CREATE TABLE IF NOT EXISTS interacciones (
    id SERIAL PRIMARY KEY,
    tipo TEXT,            -- 'seguimiento' | 'video_support'
    item_id INTEGER,      -- id en tabla seguimientos o videos
    actor_id BIGINT,      -- quien apoya
    owner_id BIGINT,      -- dueño del contenido
...
# --- Ver seguimientos (sin bloqueo diario) ---
...
async def save_seguimiento(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("SELECT tiktok_user, balance FROM users WHERE telegram_id=%s", (user_id,))
    row = cursor.fetchone()
    if not row:
    ...
    tiktok_user, balance = row["tiktok_user"], row["balance"]
    if balance < 3:
    ...
    cursor.execute("INSERT INTO seguimientos (telegram_id, link) VALUES (%s, %s)", (user_id, link))
    conn.commit()

    cursor.execute("UPDATE users SET balance = balance - 3 WHERE telegram_id=%s", (user_id,))
    conn.commit()

    cursor.execute("INSERT INTO movimientos (telegram_id, detalle, puntos) VALUES (%s, %s, %s)", (user_id, "Subir seguimiento", -3))
    conn.commit()

    await update.message.reply_text("✅ Tu seguimiento se subió con éxito.", reply_markup=back_to_menu_keyboard())
    context.user_data["state"] = None

    # Publicación opcional a canal
    try:
        await context.bot.send_message(
            chat_id="@apoyotiktok002",
            text=f"📈 Nuevo seguimiento publicado:\n👤 Usuario: @{tiktok_user}\n🔗 {link}"
        )
...
async def save_video_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("UPDATE users SET balance = balance - 5 WHERE telegram_id=%s", (user_id,))
    conn.commit()

    cursor.execute("INSERT INTO movimientos (telegram_id, detalle, puntos) VALUES (%s, %s, %s)", (user_id, "Subir video", -5))
    conn.commit()
    ...
    try:
        await context.bot.send_message(
            chat_id="@apoyotiktok002",
            text=(
                f"🎥 Nuevo video publicado:\n"
                f"👤 Usuario: @{tiktok_user}\n"
...
# --- Reclamos de apoyo y aprobación/rechazo ---
async def handle_seguimiento_done(query, context: ContextTypes.DEFAULT_TYPE, seg_id: int):
    actor_id = query.from_user.id
    cursor.execute("SELECT telegram_id FROM seguimientos WHERE id=%s", (seg_id,))
    row = cursor.fetchone()
    if not row:
    ...
    cursor.execute("SELECT id, status FROM interacciones WHERE tipo='seguimiento' AND item_id=%s AND actor_id=%s", (seg_id, actor_id))
    exists = cursor.fetchone()
    if exists:
        _, status = exists["id"], exists["status"]
    ...
    cursor.execute("""
        INSERT INTO interacciones (tipo, item_id, actor_id, owner_id, status, puntos)
        VALUES ('seguimiento', %s, %s, %s, 'pending', %s)
        RETURNING id
    """, (seg_id, actor_id, owner_id, PUNTOS_APOYO_SEGUIMIENTO))
    inter_id = cursor.fetchone()["id"]
    conn.commit()

    await query.edit_message_text("🟡 Listo, se notificó al dueño para aprobación.", reply_markup=back_to_menu_keyboard())

    cursor.execute("SELECT tiktok_user FROM users WHERE telegram_id=%s", (actor_id,))
    actor_tt_row = cursor.fetchone()
    actor_tt = actor_tt_row["tiktok_user"] if actor_tt_row and actor_tt_row["tiktok_user"] else f"{actor_id}"
    keyboard = [
...
async def handle_video_support_done(query, context: ContextTypes.DEFAULT_TYPE, vid_id: int):
    actor_id = query.from_user.id
    cursor.execute("SELECT telegram_id FROM videos WHERE id=%s", (vid_id,))
    row = cursor.fetchone()
    if not row:
    ...
    cursor.execute("SELECT id, status FROM interacciones WHERE tipo='video_support' AND item_id=%s AND actor_id=%s", (vid_id, actor_id))
    exists = cursor.fetchone()
    if exists:
        _, status = exists["id"], exists["status"]
    ...
    cursor.execute("""
        INSERT INTO interacciones (tipo, item_id, actor_id, owner_id, status, puntos)
        VALUES ('video_support', %s, %s, %s, 'pending', %s)
        RETURNING id
...
async def approve_interaction(query, context: ContextTypes.DEFAULT_TYPE, inter_id: int):
    cursor.execute("SELECT tipo, actor_id, owner_id, status, puntos FROM interacciones WHERE id=%s", (inter_id,))
    row = cursor.fetchone()
    if not row:
        await query.edit_message_text("❌ Interacción no encontrada.", reply_markup=back_to_menu_keyboard())
        return
    tipo, actor_id, owner_id, status, puntos = row["tipo"], row["actor_id"], row["owner_id"], row["status"], row["puntos"]
    ...
    cursor.execute("UPDATE interacciones SET status='accepted' WHERE id=%s", (inter_id,))
    conn.commit()
    cursor.execute("UPDATE users SET balance = balance + %s WHERE telegram_id=%s", (puntos, actor_id))
    conn.commit()
    cursor.execute("INSERT INTO movimientos (telegram_id, detalle, puntos) VALUES (%s, %s, %s)",
...
async def reject_interaction(query, context: ContextTypes.DEFAULT_TYPE, inter_id: int):
    cursor.execute("SELECT tipo, actor_id, owner_id, status FROM interacciones WHERE id=%s", (inter_id,))
    row = cursor.fetchone()
    if not row:
        await query.edit_message_text("❌ Interacción no encontrada.", reply_markup=back_to_menu_keyboard())
        return
    tipo, actor_id, owner_id, status = row["tipo"], row["actor_id"], row["owner_id"], row["status"]
    ...
    cursor.execute("UPDATE interacciones SET status='rejected' WHERE id=%s", (inter_id,))
    conn.commit()
...
# --- Comando directo para probar balance ---
...
# --- Main ---
def main():
    token = os.getenv("8518259189:AAHyjnNnep8ChXabCFka_Y809Pe0t8luxs8")
    if not token:
